...
do_install() {
    ...
    install -m 0755 ${UNPACKDIR}/fpga-bitstream ${D}${datadir}/mender/modules/v3/fpga-bitstream

    install -d ${D}${bindir}
    install -m 0755 ${UNPACKDIR}/fpga-load.sh ${D}${bindir}/fpga-load.sh

    install -d ${D}${systemd_system_unitdir}
    install -m 0644 ${UNPACKDIR}/fpga-load-on-boot.service ${D}${systemd_system_unitdir}/fpga-load-on-boot.service
}
